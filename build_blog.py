...
def protect_math(text: str) -> tuple[str, list[tuple[str, str]]]:
    """
    Replace $$...$$ and $...$ with deterministic placeholders before
    markdown parsing.

    This prevents python-markdown from interpreting LaTeX characters
    (_, *, |, {}) as formatting. Display math ($$) is processed first
    to avoid partial matches with inline math ($). Placeholders are
    content hashes (not random UUIDs) so repeated builds produce
    byte-identical output — including heading ids generated by the
    toc extension when a heading contains math.
    """
    placeholders: list[tuple[str, str]] = []

    def _placeholder(prefix: str, m: re.Match) -> str:
        ph = prefix + hashlib.md5(m.group(0).encode("utf-8")).hexdigest()
        placeholders.append((ph, m.group(0)))
        return ph

    # Display math first ($$...$$) — may span multiple lines
    text = re.sub(r"\$\$(.+?)\$\$",
                  lambda m: _placeholder("MATHBLOCK", m), text, flags=re.DOTALL)

    # Inline math ($...$) — single line, non-greedy
    text = re.sub(r"(?<!\$)\$(?!\$)(.+?)(?<!\$)\$(?!\$)",
                  lambda m: _placeholder("MATHINLINE", m), text)
