...
![Bar chart comparing a DummyClassifier and a LogisticRegression across six metrics: the DummyClassifier reaches accuracy 0.999 but Precision, Recall, and F1 are all zero, AUC-ROC is 0.500, and AUC-PR matches the base rate 0.001; LogisticRegression trades accuracy (0.926) for Recall 0.850 and AUC-PR 0.142.](../figures/not-all-errors-cost-the-same/exp_a_accuracy_trap.png)
...
![ROC curves for three classifiers with compressed AUC-ROC values (Logistic Regression 0.951, Random Forest 0.917, weak classifier 0.876); all appear competitive and the ranking favours Logistic Regression.](../figures/not-all-errors-cost-the-same/exp_b_roc_curves.png)

![Precision-Recall curves for the same three classifiers with the base-rate baseline marked; AUC-PR values are far more spread (Random Forest 0.426, weak classifier 0.409, Logistic Regression 0.122) and the ranking is reversed relative to ROC space.](../figures/not-all-errors-cost-the-same/exp_b_pr_curves.png)
...
![Threshold sweep for a LogisticRegression: Precision, Recall, and F1 as functions of the decision threshold, with vertical markers at the default (0.5), F1-optimal (0.990), and cost-optimal (0.024) thresholds; Precision and F1 stay near zero until the threshold exceeds 0.9.](../figures/not-all-errors-cost-the-same/exp_c_threshold_sweep.png)

![Row-normalised confusion matrices at the three thresholds: at 0.5, Recall 0.850 and Precision 0.011 (1,470 false positives); at 0.990, Recall 0.400 and Precision 0.078 (94 false positives); at 0.024, Recall 0.900 and Precision 0.003 (5,824 false positives).](../figures/not-all-errors-cost-the-same/exp_c_confusion_matrices.png)
...
![Precision at five recall targets (left, declining from 0.072 at r=0.75 to 0.011 at r=0.95) and false-positive/false-negative counts with expected-cost annotations (right), showing costs near \$2,000 below r=0.85 and jumping to \$8,000 above r=0.90.](../figures/not-all-errors-cost-the-same/exp_d_precision_at_recall.png)
...
![Heatmap of five model variants across seven metrics with per-cell values and ranks; orange-bordered cells mark where the top model differs from the F1 ranking, concentrated at the P@75R column where RF depth-5 beats the F1-best RF depth-15.](../figures/not-all-errors-cost-the-same/exp_e_ranking_comparison.png)
